...
describe Minefield do
  ...
  describe '#new' do
    describe 'takes a board size argument' do
      it 'defaults to a 10 by 10 board' do
        expect(m.size).to eq 10
      end
      it 'can take an integer as the size' do
        expect(size25.size).to eq 25
      end
  ...
  describe '#field' do
    ...
    it 'has as many rows as the board size' do
      expect(m.field.length).to eq m.size
    end
    it 'each row is as long as the board size' do
