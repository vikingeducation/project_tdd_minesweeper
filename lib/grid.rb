  ...
  def import_grid ( new_grid )
    ...
    (0..@width - 1).each do | row_index |
      (0..@height - 1).each do | col_index |
        @grid[row_index][col_index] = Square.new("unmarked")
        if new_grid[row_index][col_index] == "*"
          @grid[row_index][col_index].status = "bomb"
        else
          @grid[row_index][col_index].status = "no_bomb"
        end
      end
    end
  end
  ...
  def place_bombs
    @grid.each_with_index do |row, i|
      row.each_with_index do |column, j|
        @mines_hash.include?([i,j]) ? @grid[i][j].status = "bomb" : @grid[i][j].status = "no_bomb"
      end
    end
  end
  ...
  def calculate_adjacent_bombs( i, j )  
    ...
    horizontal_arr.each do | row_index |
      vertical_arr.each do | col_index |
        num_adjacent_bombs += 1  if @grid[row_index][col_index].status =="bomb"
      end
    end
    @grid[i][j].status =="bomb" ? num_adjacent_bombs - 1 : num_adjacent_bombs
  end
